  ...
  return (
    <article className={`project${index % 2 === 1 ? " project--reverse" : ""}`}>
      <div className="project__media">
        {image ? (
          <img src={`./images/myscreenshots/${image}`} alt={project.title || project.name} />
        ) : (
          <div className="project__media--empty">{project.title || project.name}</div>
        )}
      </div>
      ...
      <div className="project__content">
        <div className="project__head">
          <h2 className="project__title">
            ...
            {featured && <span className="project__featured"> — featured</span>}
          </h2>
          <span className="project__year">{number} / {total}{project.year ? ` — ${project.year}` : ""}</span>
        </div>
        ...
        {stack.length > 0 && <p className="project__stack">{stack.join(" · ")}</p>}

        <div className="project__links">
          {liveUrl && <a href={liveUrl} target="_blank" rel="noreferrer">View live ↗</a>}
          {repoUrl && <a href={repoUrl} target="_blank" rel="noreferrer">Source ↗</a>}
          {!liveUrl && !repoUrl && <span>Links coming soon</span>}
        </div>
      </div>
    </article>
  );
  ...
  return (
    <div className="projects">
      ...
      <header className="projects-head">
        ...
        <p className="projects-head__meta">
          {projects.length} selected pieces of work, spanning coursework, competitions, and things
      ...
      <div className="project-list">
        {projects.map((project, i) => (
          <ProjectRow key={project.id || i} project={project} index={i} total={projects.length} />
        ))}
      </div>

      <p className="projects-more">
        <strong>More in progress.</strong> A few things currently being built aren't listed here yet.
      </p>
    </div>
  );
}